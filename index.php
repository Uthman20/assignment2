
<link rel="stylesheet" href="css/styles.css">
<section id="header1">
    <img src="images/1.png" alt="">

</section>

<section id="header2">
    <img src="images/2.png" alt="">

</section>
 

<ul class="navigation">
    <li><a href="index.php">Home</a></li>
    <li><a href="appointments.php">Appointments</a></li>
    <li><a href="brands&awards.php">Brands & Awards</a></li>
    <li><a href="business.php">Business</a></li>
    <li><a href="economy.php"></a>Economy</li>
    <li><a href="investors&analysis">Investors & Analysis</a></li>
    <li><a href="markets.php">Markets</a></li>
    <li><a href="rankings&data.php">Rankings & Data</a></li>
    <li><a href="regulators.php">Regulators</a></li>

</ul>


<section id="header3">
<img src="images/img3.png" alt="">
</section>

  <!--HEADLINE STORIES SECTION STARTS HERE -->  
<section class="headline" id="headline">
<h3><strong>-HEADLINE STORIES</strong></h3>
    <div class="container">
        

        <DIV class="box1">
          <div class="box1overlay"></div> 
          
          <div class="box1content">
            <button>OPINION</button>
            <h3><a href="#"> president Tinubu's First year: The Fraught State Of Nigeria's Economy</a></h3>

          </div>
          

        </DIV>

        <DIV class="doublebox">
            <div class="box2">
            <div class="box2overlay"></div>
            <div class="box2content">
                <button>MARKETS</button>
                <H3>Guaranty Trust Holding Company (GTCO) Plc Gains Shareholder Approval for $750 Million Capital Raise</H3>
            </div>
            </div>

            <div class="box3">
                <div class="box3overlay"></div>
                <div class="box3content">
                    <button>APPOINTMENTS</button>
                    <H3>Beta Glass Plc Announces Changes to its Board of Directors</H3>
                </div>
            </div>

        </DIV>

        <div class="box4">
            <img src="images/ECO-AD.jpg" alt="">
        </div>

    </div>

</section>

<!--HEADLINE STORIES SECTION STARTS HERE -->


<!--UNILEVER SECTION STARTS HERE -->
<section class="heady" id="heady">
    <h3><strong>-TOP STORIES-</strong></h3>

<DIV class="container">
    <div class="col1">
        <img src="images/unilever.webp" alt="">
    </div>

    <div class="col2">
        <button>MARKETS</button>
        <h3>Unilever Nigeria PLC Holds Annual General Meeting, Announces Key Resolutions</h3>

        <p>May 11,2024</p>
        <p>Unilever Nigeria PLC declared a dividend totaling N4.3 billion equivalent to N0.75k per share, for the fiscal year ending 31st December 2023.</p>

    </div>

    

</DIV>

</section>
<!--UNILEVER SECTION ENDS HERE -->

<!--TOTAL SECTION STARTS HERE -->
<section class="total" id="total">
    <div class="container">
        <div class="col1">
            <img src="images/Total.webp" alt="">

        </div>

        <div class="col2">
            <button>BRANDS & AWARDS</button>
            <H3>TotalEnergies Launches 2024 Startuppers Challenge to Empower African Entrepreneurs</H3>
            <p>May 11,2024</p>
            <p>TotalEnergies Nigeria has unveiled the 2024 edition of its Startuppers challenge, geared towards supporting businesses in Nigeria and across Africa.</p>


        </div>

    </div>

</section>
<!--TOTAL SECTION ENDS HERE -->

<!--APPOINTMENT SECTION STARTS HERE -->
<section class="appointment" id="appointment">
    <div class="container">
        <div class="col1">
            <img src="images/pappi.webp" alt="">

        </div>

        <div class="col2">
            <button>APPOINTMENTS</button>
            <h3>FirstBank Welcomes Ebenezer Oluwofose as Chairman Following Tunde Hassan-Odukale’s Retirement</h3>
            <p>MAY 11,2024</p>
            <P>Mr Ebenezer Oluwofose has been appointed as the new Chairman of the Board of Directors of First Bank of Nigeria Limited.</P>


        </div>

    </div>


</section>
<!--APPOINTMENT SECTION ENDS HERE -->

<!--APPOINTMENT2 SECTION STARTS HERE -->
<section class="appointment2" id="appointment2">
    <div class="container">
        <div class="col1">
            <img src="images/CFO.jpg" alt="">

        </div>
        <div class="col2">
            <button>APPOINTMENTS</button>
            <h3>Mr. Ogaga Ologe Resigns as Finance Director of Cadbury Nigeria Plc</h3>
            <p>MAY 11, 2024</p>
            <P>Mr. Ogaga Ologe’s resignation as Cadbury Nigeria Plc’s Finance Director, effective from July 22nd, 2024.</P>


        </div>

    </div>

</section>
<!--APPOINTMENT2 SECTION ends HERE -->

<!--brands n awards SECTION STARTS HERE -->
<section class="brands" id="brands">
    <div class="container">
        <div class="col1">
            <img src="images/access.webp" alt="">
        </div>
        <div class="col2">
            <button>BRANDS & AWARDS</button>
            <H3>
            Access Bank Group, Mastercard Partner To Enhance Cross-Border Payments In Africa
            </H3>
            <P>MAY 9, 2024</P>
            <P>Access Bank in partnership with Mastercard has launched a solution to expand access to cross-border payments and remittances in Africa.</P>


        </div>

    </div>

</section>
<!--brands n awards SECTION STARTS HERE -->

<!--REGULATORS SECTION STARTS HERE -->
<section class="regulator" id="regulator">
    <div class="container">
        <div class="col1">
            <img src="images/CBN-headquarters.jpg" alt="">

        </div>
        <div class="col2">
            <button>REGULATORS</button>
            <H3>CBN Extends Cash Deposit Processing Fee Suspension Until September 30, 2024</H3>
            <p>MAY 9,2024</p>
            <P>The suspension covers charges imposed on cash deposits above N500,000 for individuals and N3,000,000 for corporates.</P>

        </div>

    </div>

</section>
<!--REGULATORS SECTION ENDS HERE -->

<!--APPOINTMENTS SECTION STARTS HERE -->
<section class="appointment3" id="appointment3">
    <div class="container">
        <div class="col1">
            <img src="images/gal.jpg" alt="">

        </div>
        <div class="col2">
            <button>APPOINTMENTS</button>
            <H3>Ifunanya Obiakor Promoted to Head West Africa Marketing Role at Arla Foods</H3>
            <P>MAY 8, 2024</P>
            <P>Ifunanya Obiakor, has ascended to greater heights in her professional journey as the Head of Marketing, West Africa, Arla Foods.</P>

        </div>

    </div>

</section>
<!--APPOINTMENTS SECTION ENDS HERE -->

<!--INFLATION SECTION STARTS HERE -->
<section class="inflation" id="inflation">
    <div class="container">
        <div class="col1">
            <img src="images/inflation.jpg" alt="">

        </div>
        <div class="col2">
            <button>OPINION</button>
            <H3>Is Inflation getting out of hands?</H3>
            <P>MAY 8, 2024</P>
            <P>A raise in the minimum wage by labor unions is arguably right, but at what price will they settle? Are Nigerians being told to survive and not prosper?</P>


        </div>

    </div>

</section>
<!--INFLATION SECTION ENDS HERE -->














<?php include 'include/footer.php';?>






 

























</body>
</html>